...
(realproof)
(assume "p")
(inst-with-to "RealEqCharOneRealConstrFree"
...
(add-computation-rule
 "ContComp f0 f"
 "ContConstr (f doml) (f domr) ([a, n] f0 approx (f approx a n) n) ([p] (f0 uMod (p+1)) max (f uMod (PosPred(f0 uModCont (p+1))))) ([p] f uModCont (PosPred(f0 uModCont p))) (f0 lb) (f0 ub)")

(set-totality-goal "ContComp")
(assume "f^0" "Tf0")
(elim "Tf0")
(assume "a^0" "Ta0" "b^0" "Tb0" "h^0" "Th0" "al^0" "Tal0" "om^0" "Tom0"
	"mu^0" "Tmu0" "nu^0" "Tnu0")
(assume "f^" "Tf")
(elim "Tf")
(assume "a^" "Ta" "b^" "Tb" "h^" "Th" "al^" "Tal" "om^" "Tom" "mu^" "Tmu" "nu^" "Tnu")
(ng #t)
(intro 0)
(use "Ta")
(use "Tb")
...
;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; ;; 2020-07-19.  Updated following libcont20.scm, up to ContLim and ContRat.
