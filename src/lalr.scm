...
(define-syntax BITS-PER-WORD (syntax-rules () ((BITS-PER-WORD) 28)))
;  (def-macro (logical-or x . y) `(fxlogor ,x ,@y))
;(define-syntax logical-or (syntax-rules ()
;			    ((logical-or x y ) (fxlogor x y))))

;;; ---------- END OF SYSTEM DEPENDENT SECTION ------------

;; schimans 11/04/05: use SLIB logical/bitwise operations
(load "logical.scm")
...
; (define-syntax set-bit
;   (syntax-rules ()
;     ((set-bit v b )
;      (let ((x (quotient b (BITS-PER-WORD)))
; 	   (y (fxsll 1 (remainder b (BITS-PER-WORD)))))
;        (vector-set! v x (logical-or (vector-ref v x) y))))))
...
(define-syntax new-core (syntax-rules () ((new-core) (make-vector 4 0))))
;(def-macro (set-core-number! c n)  `(vector-set! ,c 0 ,n))
(define-syntax set-core-number!
  (syntax-rules () ((set-core-number! c n) (vector-set! c 0 n))))
;(def-macro (set-core-acc-sym! c s) `(vector-set! ,c 1 ,s))
(define-syntax set-core-acc-sym!
  (syntax-rules () ((set-core-acc-sym! c s) (vector-set! c 1 s))))
;(def-macro (set-core-nitems! c n)  `(vector-set! ,c 2 ,n))
(define-syntax set-core-nitems!
  (syntax-rules () ((set-core-nitems! c n) (vector-set! c 2 n))))
;(def-macro (set-core-items! c i)   `(vector-set! ,c 3 ,i))
(define-syntax set-core-items!
  (syntax-rules () ((set-core-items! c i) (vector-set! c 3 i))))
;(def-macro (core-number c)         `(vector-ref ,c 0))
(define-syntax core-number
  (syntax-rules () ((core-number c ) (vector-ref c 0))))
;(def-macro (core-acc-sym c)        `(vector-ref ,c 1))
(define-syntax core-acc-sym
  (syntax-rules () ((core-acc-sym c ) (vector-ref c 1))))
;(def-macro (core-nitems c)         `(vector-ref ,c 2))
(define-syntax core-nitems
  (syntax-rules () ((core-nitems c ) (vector-ref c 2))))
;(def-macro (core-items c)          `(vector-ref ,c 3))
(define-syntax core-items
  (syntax-rules () ((core-items c ) (vector-ref c 3))))

;(def-macro (new-shift)              `(make-vector 3 0))
(define-syntax new-shift
  (syntax-rules () ((new-shift-) (make-vector 3 0))))
;(def-macro (set-shift-number! c x)  `(vector-set! ,c 0 ,x))
(define-syntax set-shift-number!
  (syntax-rules () ((set-shift-number! c x) (vector-set! c 0 x))))
;(def-macro (set-shift-nshifts! c x) `(vector-set! ,c 1 ,x))
(define-syntax set-shift-nshifts!
  (syntax-rules () ((set-shift-nshifts! c x) (vector-set! c 1 x))))
;(def-macro (set-shift-shifts! c x)  `(vector-set! ,c 2 ,x))
(define-syntax set-shift-shifts!
  (syntax-rules () ((set-shift-shifts! c x) (vector-set! c 2 x))))
;(def-macro (shift-number s)         `(vector-ref ,s 0))
(define-syntax shift-number
  (syntax-rules () ((shift-number s) (vector-ref s 0))))
;(def-macro (shift-nshifts s)        `(vector-ref ,s 1))
(define-syntax shift-nshifts
  (syntax-rules () ((shift-nshifts s ) (vector-ref s 1))))
;(def-macro (shift-shifts s)         `(vector-ref ,s 2))
(define-syntax shift-shifts
  (syntax-rules () ((shift-shifts s) (vector-ref s 2))))

;(def-macro (new-red)                `(make-vector 3 0))
(define-syntax new-red
  (syntax-rules () ((new-red) (make-vector 3 0))))
;(def-macro (set-red-number! c x)    `(vector-set! ,c 0 ,x))
(define-syntax set-red-number!
  (syntax-rules () ((set-red-number! c x) (vector-set! c 0 x))))
;(def-macro (set-red-nreds! c x)     `(vector-set! ,c 1 ,x))
(define-syntax set-red-nreds!
  (syntax-rules () ((set-red-nreds! c x) (vector-set! c 1 x))))
;(def-macro (set-red-rules! c x)     `(vector-set! ,c 2 ,x))
(define-syntax set-red-rules!
  (syntax-rules () ((set-red-rules! c x) (vector-set! c 2 x))))
;(def-macro (red-number c)           `(vector-ref ,c 0))
(define-syntax red-number
  (syntax-rules () ((red-number c) (vector-ref c 0))))
;(def-macro (red-nreds c)            `(vector-ref ,c 1))
(define-syntax red-nreds
  (syntax-rules () ((red-nreds c) (vector-ref c 1))))
;(def-macro (red-rules c)            `(vector-ref ,c 2))
(define-syntax red-rules
  (syntax-rules () ((red-rules c) (vector-ref c 2))))


;(def-macro (new-set nelem)  `(make-vector ,nelem 0))
(define-syntax new-set
  (syntax-rules () ((new-set nelem) (make-vector nelem 0))))
...
(define (gen-lalr1 gram output-file . opt)
  ...
  (rewrite-grammar
   gram
   (lambda (terms vars gram gram/actions terminal-table literal-code)
	 ...
	 (with-output-to-file output-file
	   (lambda ()
	     (display "; This file is automaticaly generated. Do not edit!")
	     (newline)
	     (newline)
	     ...
	     (display "; *** Terminals Table ***")
	     (newline)
	     (output-terminal-table terms terminal-table prefix)
	     (display "; *** Lexer Definition ***")
	     (newline)
	     (output-lexer-def parser-prefix prefix)
	     (newline)
	     (display "; *** Shortcut Definition ***")
	     (newline)
	     (output-shortcut-def parser-prefix prefix)
	     (newline)
	     (display "; *** Add-Token Definition ***")
	     (newline)
	     (output-add-token-def parser-prefix prefix)
	     (display "; *** Remove-Token Definition ***")
	     (newline)
	     (output-remove-token-def parser-prefix prefix)
	     (display "; *** Is-Token? Definition ***")
	     (newline)
	     (output-is-token-def parser-prefix prefix)
	     (display "; *** Display-Tokens Definition ***")
	     (newline)
	     (output-display-tokens-def parser-prefix prefix)
	     )))))))
...
(define (output-add-token-def parser-prefix prefix)
  (display "(define ") (display parser-prefix) (display "add-token") (newline)
  (display "   (make-add-token") (newline)
  (display "    ") (display prefix) (display "terminal-table") (newline)
  (display "    ") (display prefix) (display "token-nrs") (newline)
  (display "))") (newline)
  (newline))

(define (output-remove-token-def parser-prefix prefix)
  (display "(define ") (display parser-prefix)
  (display "remove-token") (newline)
  (display "   (make-remove-token") (newline)
  (display "    ") (display prefix) (display "terminal-table") (newline)
  (display "    ") (display prefix) (display "token-nrs") (newline)
  (display "))") (newline)
  (newline))

(define (output-is-token-def parser-prefix prefix)
  (display "(define ") (display parser-prefix)
  (display "is-token?") (newline)
  (display "   (make-is-token?") (newline)
  (display "    ") (display prefix) (display "terminal-table") (newline)
  (display "    ") (display prefix) (display "token-defs") (newline)
  (display "))") (newline)
  (newline))

(define (output-display-tokens-def parser-prefix prefix)
  (display "(define (") (display parser-prefix) (display "display-tokens)") (newline)
  (display "   (lexer-info") (newline)
  (display "    ") (display prefix) (display "terminal-table") (newline)
  (display "    ") (display prefix) (display "token-nrs") (newline)
  (display "))") (newline)
  (newline))
...
(define (output-lexer-def parser-prefix prefix)
  (display "(define ") (display parser-prefix) (display "lexer") (newline)
  (display "   (make-lexer") (newline)
  (display "    ") (display prefix) (display "terminal-table") (newline)
  (display "    ") (display prefix) (display "token-nrs") (newline)
  (display "))") (newline)
  (newline))

(define (output-shortcut-def parser-prefix prefix)
  (display "(define (") (display parser-prefix) (display "pf string)")
  (newline)
  (display "   (") (display parser-prefix) (display "parser (")
  (display parser-prefix) (display "lexer")
  (display " (string-reader string)) myerror))") (newline)
  (newline)
  (newline)
  (display "(define (") (display parser-prefix)
  (display "parse-file filename)")
  (newline)
  (display "  (let ((parse-port") (newline)
  (display "    (lambda (port)") (newline)
  (display "      (") (display parser-prefix) (display "parser (")
  (display parser-prefix) (display "lexer")
  (display " (port-reader filename port)) myerror))))") (newline)
  (display "    (call-with-input-file filename parse-port)))") (newline)
  (newline))


(define (output-token-defs terms prefix)
  (display "(define ") (display prefix) (display "token-nrs") (newline)
...
(define (element->nr e l i)
  (if (pair? l)
	  (if (equal? (car l) e)
		  i
		  (element->nr e (cdr l) (+ i 1)))
	  (error "Undefined token type" (symbol->string e))))

(define (output-terminal-table terms terminal-table prefix)
  (let* ((hsize 97)
		(hashindex (lambda (char-list hsize)
		...
		(hinsert (lambda (table string nr value)
				   (let ((hindex (hashindex (string->list string) hsize)))
					 (vector-set! table hindex
								  (cons (cons string (cons nr value))
										(vector-ref table hindex))))))
		(htable (let ((table (make-vector hsize '())))
				  (do ((l terminal-table (cdr l)))
					  ((null? l))
					(hinsert  table (caar l) (element->nr (cadar l) terms 0) (cddar l)))
				  table)))
  (newline)
  (display "(define ") (display prefix) (display "terminal-table") (newline)
  (display "   `#") (display "(") (newline)
  (do ((i 0 (+ i 1)))
      ((>= i hsize))
      (display "   (")
      (do ((l (vector-ref htable i) (cdr l)))
	  ((null? l) (display ")") (newline))
	(display "(")
	(write (caar l))
	(display " ")
	(write (cadar l))
...
(define (rewrite-grammar grammar proc)
  ...
  (if (not (pair? grammar))
      ...
      ;; Modified since petite chez scheme gives warning.
      (error "Grammar definition must be a non-empty list" "")
      (let ((literal-code (car grammar)))
      (let loop1 ((lst (cadr grammar)) (rev-terms '()) )
	(if (pair? lst) ; definition d'un terminal?
	    (let ((term (car lst)))
	      (cond ((not (valid-terminal? term))
		     (error "Invalid terminal:" (symbol->string term)))
		    ((member term rev-terms)
		     (error "Terminal previously defined:" (symbol->string term)))
		    (else
		     (loop1 (cdr lst)
			    (cons term rev-terms)))))

      (let loop1b ((lst (caddr grammar)) (rev-term-table '()) )
	(if (pair? lst) ; definition d'un token?
	    (let ((term (cadar lst)))
	      (cond ((not (member term rev-terms))
		     (error "Tokentype undefined:" (symbol->string term)))
		    ((assoc (caar lst) rev-term-table)
		     (error "Token defined twice:" (caar lst)))
		    (else
		     (loop1b (cdr lst)
			     (cons (car lst) rev-term-table)))))

      (let loop2 ((lst (cdddr grammar)) (rev-nonterm-defs '()))
	      (if (pair? lst)
		  (let ((def (car lst)))
		    (if (not (pair? def))
			(error "Nonterminal definition must be a non-empty list" def)
			(let ((nonterm (car def)))
			  (cond ((not (valid-nonterminal? nonterm))
				 (error "Invalid nonterminal:"
					(symbol->string nonterm)))
		  ...
		  (let* ((terms (cons eoi (reverse rev-terms)))
                         (term-table (reverse rev-term-table))
	...
	(let loop1 ((lst (cdr nonterm-def))
	  ...
	  (if (not (pair? lst))
		...
		(for-each (lambda (x)
				...
				(error "Invalid terminal or nonterminal"
				       (symbol->string x))))
			  rhs)
		(if (and (pair? rest)
			 (eq? (car rest) ':)
