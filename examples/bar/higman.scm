;; 2015-01-03.  higman.scm.  Based on higman141227.scm.  Changes in
;; pconst.scm formula.scm axiom.scm proof.scm pproof.scm done.

;; Based on higaf.scm.  Based on higaf141215.scm.  ESeqA replaces
;; ExtsA.  Superfluous wqo removed in Exts and ExtsA.

;; Earlier comments.  higaf.scm.  Based on higaf141213.scm.  InsertATN
;; defined with LargerARExAll for ta with ExtAT ta.  Then always
;; (InsertATN ta a =/= ta), and moreover InsertAFN tas a =/= tas if
;; LargerAR w heads roots tas.  Properties of LargerARExAll taken from
;; higafneloc.scm

;; Earlier comments: Based on higafne.scm, but the definition
;; of BarFN is changed: LargerAR a heads roots tas instead of
;; (InsertAFN wqo tas a=tas -> F).  Relations between both conditions
;; are in InsertAFNeq and InsertAFNId.  The previous higaf141202 had
;; LExAll wqo a Roots tas.

;; higafne.scm was based on higwqo.scm, keeping its terminology.
;; Difference: No transitivity and a changed InsertFN with comparison
;; of a with all member of the sequence (rather than its head).  BarFN
;; is defined with inequality, i.e., predecessors of BarFN ts are all
;; (InsertFN wqo ts v a) different from ts.

;; The development is temporary, with only the definitions complete
;; (but no totality proofs and usage of admit as convenient).

;; Cleanup in notation for unary destructors (also iterated ones):
;; prefix notation with lower-case initial character (unimportant)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; In archive/wiesnet/Implementierung.scm

;; LargerARAll
;; IncrN
;; InsertFN (with LargerARAll) (or InsertWald)
;; InsertTN (or InsertBaum)
;; ForestN (or Wald)
;; Exts ((Exts wqo vsass vsas) means each vsas1 in vsass is a one-element 
;;   extension of vsas)
;; CorrTN (with IncrN)
;; CorrFN

;; ;; Lemma 2.18 (~ CorrConsAux in higwqo)
;; (set-goal "all wqo,ws,w,n(
;;  all t(HtT t < n -> CorrTN wqo ws t -> CorrTN wqo (w::ws) t) &
;;  all ts(HtF ts <n -> CorrFN wqo ws ts -> CorrFN wqo (w::ws) ts))")

;; ;; InsertFNCons (immediate from definitions)
;; (set-goal "all wqo^,a^,ws^,v^(
;;  H wqo^ a^ ws^ -> ForestN wqo^((a^ ::v^)::ws^)eqd
;;                   InsertFN wqo^(ForestN wqo^ ws^)v^ a^)")

;; ;; CorrInsertNAux (~ CorrInsertAux)
;; (set-goal "all wqo,ws,a,v,n(
;;  all t(HtT t<n -> CorrTN wqo ws t -> LargerARAll wqo a rht root t ->
;;   CorrTN wqo((a::v)::ws)(InsertTN wqo t v a)) &
;;  all ts(HtF ts<n -> CorrFN wqo ws ts ->
;;   CorrFN wqo((a::v)::ws)(InsertFN wqo ts v a)))")

;; ;; CorrFNForestFN (~ CorrFForest)
;; (set-goal "all wqo,ws(CorrFN wqo ws (ForestN wqo ws))")
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; 2015-01-03.  Added to git.
;; ;; In pconst.scm

;; ;; Log text

;; ;; change-t-deg-to-one adapted to simultaneous definitions of pconsts:
;; ;; it now takes a list of pconst-names as arguments.  This is
;; ;; necessary to have all these pconsts in the stored totality theorems
;; ;; with t-deg one.
...
;; ;; proof argument or the proof under construction is for a conjunction
;; ;; of totality formulas for simultaneously defined program constants
;; ;; of t-deg-zero.  It changes the degree of totality for all these to
...
;; ;; t-deg-one, and can be useful in later totality proofs.

;; ;; End of log text

;; ;; (add-totality-theorems . opt-proof) expects that the proof argument
;; ;; or the proof under construction is for a conjunction of totality
;; ;; formulas for simultaneously defined program constants of
...
;; ;; In nat.scm (or lib/boole.scm)

;; ;; Log text

;; ;; EqFalseToNeg NegToEqFalse NegToEqFalse BooleAeqToEq OrIntroLeft
;; ;; OrIntroRight OrElim IfAndb IfOrb added (would fit better into a
;; ;; file lib/boole.scm)
...
;; 2015-01-03.  Keep convention that destructors with lower case
;; initial character are only left right lft rht.

;; (remove-theorem "ListHeadTotal")
...
;; ListTailsTotal
(set-goal
 (rename-variables (term-to-totality-formula (pt "(ListTails alpha)"))))
(assume "(list list alpha)^" "Txss")
(elim "Txss")
(use "TotalListNil")
(assume "xs^" "Txs" "(list list alpha)^1" "Txss1" "IH")
...
;; ListRhtsTotal
(set-goal
 (rename-variables (term-to-totality-formula (pt "(ListRhts alpha1 alpha2)"))))
(assume "yzs^" "Tyzs")
(elim "Tyzs")
(use "TotalListNil")
...
;; ListLftsTotal
(set-goal
 (rename-variables (term-to-totality-formula (pt "(ListLfts alpha1 alpha2)"))))
(assume "yzs^" "Tyzs")
(elim "Tyzs")
(use "TotalListNil")
(assume "yz^" "Tyz" "yzs^1" "Tyzs1" "IH")
